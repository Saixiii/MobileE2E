        ...
        try {
            ...
            while (namingEnum.hasMore()) {
                ...
                while (namingAttr.hasMoreElements()) {
                    ...
                    if(cusattr.get("cdb:" + attr.getID()) == null)
                        cusattr.put("cdb:" + attr.getID() , attr.get(0));
                    else
                        cusattr.put("cdb:" + attr.getID() , cusattr.get("cdb:" + attr.getID()) + "|" + attr.get(0));
                }
            }
            namingEnum.close();
            cusattr.put("cdb:rs","success");
            
        } catch (NamingException ne) {
            cusattr.put("cdb:rs","Account is not found in CDB");
        } catch (Exception e) {
            cusattr.put("cdb:rs",e);
            e.printStackTrace();
        } finally {
            if (dirCtx != null) {
                try {
